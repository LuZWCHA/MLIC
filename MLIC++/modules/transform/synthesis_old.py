...
class HyperSynthesis(nn.Module):
    ...
    def __init__(self, M=192, N=192) -> None:
        ...
        self.increase = nn.Sequential(
            conv3x3(N, M, use_deep_wise_conv=False),
            nn.GELU(),
            subpel_conv3x3(M, M, 2, use_deep_wise_conv=False),
            nn.GELU(),
            conv3x3(M, M * 3 // 2, use_deep_wise_conv=False),
            nn.GELU(),
            subpel_conv3x3(M * 3 // 2, M * 3 // 2, 2, use_deep_wise_conv=False),
            nn.GELU(),
            conv3x3(M * 3 // 2, M * 2, use_deep_wise_conv=False),
        )
