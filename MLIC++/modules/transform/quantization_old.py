...
class LatentResidualPredictionOld(nn.Module):
    def __init__(self, in_dim, out_dim, act=nn.GELU):
        ...
        self.lrp_transform = nn.Sequential(
            conv3x3(in_dim, in_dim - diff // 4, use_deep_wise_conv=False),
            act(),
            conv3x3(in_dim - diff // 4, in_dim - diff // 2, use_deep_wise_conv=False),
            act(),
            conv3x3(in_dim - diff // 2, in_dim - diff * 3 // 4, use_deep_wise_conv=False),
            act(),
            conv3x3(in_dim - diff * 3 // 4, out_dim, use_deep_wise_conv=False),
        )
...
class LatentResidualPrediction(nn.Module):
    def __init__(self, in_dim, out_dim, act=nn.GELU):
        super().__init__()
        self.lrp_transform = nn.Sequential(
            conv3x3(in_dim, 224, use_deep_wise_conv=False),
            act(),
            conv3x3(224, 128, use_deep_wise_conv=False),
            act(),
            conv3x3(128, out_dim, use_deep_wise_conv=False),
        )
